...
public class SyncWritable<T> implements SyncRepository.Writable<T> {
  ...
  public WriteResult insert(Iterable<? extends T> docs) {
    return Flowable.fromPublisher(writable.insert(docs)).singleOrError().blockingGet();
  }

  @Override
  public WriteResult delete(Criterion<T> criteria) {
    return Flowable.fromPublisher(writable.delete(criteria)).singleOrError().blockingGet();
  }
}
